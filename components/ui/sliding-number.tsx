  ...
  return (
    <div style={{ height: digitHeight }} className="relative w-[1ch] overflow-hidden tabular-nums">
      {Array.from({ length: 10 }, (_, i) => (
        <NumberComp key={i} mv={animatedValue} number={i} digitHeight={digitHeight} />
      ))}
    </div>
  );
}

function NumberComp({ mv, number, digitHeight }: { mv: MotionValue<number>; number: number; digitHeight: number }) {
...
export function SlidingNumber({
  ...
  }, [from]); // Empty dependency array - runs on every mount
